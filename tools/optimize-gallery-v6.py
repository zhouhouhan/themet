...
ROOT=Path(r'\\192.168.31.246\Media\BaiduNetdiskWorkspace\myagent-work\zcode\vgallery')
bpy.ops.wm.open_mainfile(filepath=str(ROOT/'assets/models/blender/gallery-v6.blend'))
# Include fine curve mouldings and readable artwork captions in the web asset.
for ob in list(bpy.context.scene.objects):
 if ob.type in {'CURVE','FONT'}:
  if ob.type=='FONT':
   ob.data.size=.013;ob.data.text_boxes[0].width=.15
...
bpy.ops.export_scene.gltf(filepath=str(ROOT/'assets/models/gallery-v6.glb'),export_format='GLB',export_apply=True,export_cameras=True,export_lights=False,export_extras=True)
